...
static int ramoops_probe(struct platform_device *pdev)
{
	...
	/* ======= 关键修复：确保缓冲区大小有效 ======= */
	/*
	 * 即使设置了record_size，某些情况下buffer_size可能仍为0
	 * 添加额外检查确保最小缓冲区大小
	 */
	if (pdata->record_size > 0) {
		/*
		 * 计算实际可用的缓冲区大小
		 * 减去 persistent_ram 的头部开销
		 */
		size_t min_usable_size = sizeof(struct persistent_ram_buffer);
		if (pdata->record_size <= min_usable_size) {
			pr_warn("ramoops: record_size too small (%zu <= %zu), increasing to minimum\n",
				pdata->record_size, min_usable_size);
			pdata->record_size = min_usable_size + SZ_4K;
		}
	}
	...
	if (cxt->pstore.flags & PSTORE_FLAGS_DMESG) {
		...
		if (cxt->pstore.bufsize == 0) {
			pr_warn("ramoops: buffer_size is 0, setting to minimum usable size\n");
