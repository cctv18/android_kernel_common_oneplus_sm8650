			    ...
			    struct ramoops_platform_data *pdata)
{
	...
	struct resource *res;
	u32 u32_value;  // 用于32位属性
	u64 addr_val;   // 用于64位属性
	int ret;
	...
	if (!res) {
		...
		if (rmem) {
			...
			pr_info("ramoops: reserved_mem: 0x%zx@0x%llx\n",
				rmem->size, (unsigned long long)rmem->base);
		} else {
			dev_err(&pdev->dev,
		...
		pr_info("ramoops: platform resource: 0x%zx@0x%llx\n",
			resource_size(res), (unsigned long long)res->start);
	}
	...
	if (!ret) {
		...
		pr_info("ramoops: overriding address with DT 'addr' property: 0x%llx\n",
	...
	if (!ret) {
		...
		pr_info("ramoops: overriding size with DT 'size' property: 0x%llx\n",
	...
	/* 处理 u32 类型的属性 */
	if (!of_property_read_u32(of_node, "mem-type", &u32_value))
		pdata->mem_type = u32_value;
	
	if (!of_property_read_u32(of_node, "record-size", &u32_value))
		pdata->record_size = u32_value;
	
	if (!of_property_read_u32(of_node, "console-size", &u32_value))
		pdata->console_size = u32_value;
	
	if (!of_property_read_u32(of_node, "ftrace-size", &u32_value))
		pdata->ftrace_size = u32_value;
	
	if (!of_property_read_u32(of_node, "pmsg-size", &u32_value))
		pdata->pmsg_size = u32_value;
	
	if (!of_property_read_u32(of_node, "ecc-size", &u32_value))
		pdata->ecc_info.ecc_size = u32_value;
	
	if (!of_property_read_u32(of_node, "flags", &u32_value))
		pdata->flags = u32_value;
	
	if (!of_property_read_u32(of_node, "max-reason", &u32_value))
		pdata->max_reason = u32_value;
	...
	/* 调试信息 */
	pr_info("ramoops: DT parsed: 0x%zx@0x%llx\n",
...
static int ramoops_probe(struct platform_device *pdev)
{
	...
	/* ======= 关键修复：确保地址是64位 ======= */
	/* 强制将地址视为64位 */
	if (pdata->mem_address < 0x100000000ULL) {
		pr_warn("ramoops: low address detected, applying 36-bit fix\n");
		pdata->mem_address |= 0xB00000000ULL; /* 添加高位 */
	}
	...
	/* ======= 关键修复：打印完整物理地址 ======= */
	pr_info("ramoops: physical address = 0x%016llx\n", (unsigned long long)cxt->phys_addr);

	paddr = cxt->phys_addr;

	/* ======= 关键修复：确保内存大小正确 ======= */
	/* 设备树报告2304KB (0x240000) */
	if (cxt->size != 0x240000) {
		pr_warn("ramoops: mem_size mismatch, forcing to 0x240000 (was 0x%zx)\n", 
			cxt->size);
		cxt->size = 0x240000;
	}
	...
	pr_info("ramoops: dump_mem_sz = 0x%zx (0x%zx - 0x%zx - 0x%zx - 0x%zx)\n",
	...
	if (cxt->pstore.flags & PSTORE_FLAGS_DMESG) {
		...
		pr_info("ramoops: dmesg buffer size = 0x%zx\n", cxt->pstore.bufsize);
	...
	pr_info("using 0x%lx@0x%016llx, ecc: %d\n",
		cxt->size, (unsigned long long)cxt->phys_addr,
